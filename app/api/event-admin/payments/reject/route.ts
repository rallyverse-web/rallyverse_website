...
import { getRegistrationById, rejectPayment } from '@/lib/repositories/registrations'
import { getAdminFromRequest } from '@/lib/event-admin-auth'
import { getEventById } from '@/lib/repositories/events'
import { sendPaymentRejectedEmail } from '@/lib/send-email-service'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { registration_id, reason, send_email } = await req.json()
    if (!registration_id) {
    ...
    const updated = await rejectPayment(registration_id, admin.id, reason || null)
