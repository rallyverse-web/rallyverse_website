...
import { Mail, Plus, Edit, Trash2, Copy, Eye, Send, Loader2, RefreshCw, FileText, History, CheckCircle, XCircle, ArrowLeft, List, Users } from 'lucide-react'
...
type Tab = 'templates' | 'preview' | 'test-email' | 'send-email' | 'logs'

export default function EventAdminCommunicationPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    if (!eventId) return
    try {
      const res = await fetch(`/api/event-admin/email-templates/${eventId}`)
  ...
  const fetchSettings = useCallback(async () => {
    if (!eventId) return
    try {
      const res = await fetch(`/api/event-admin/email-settings/${eventId}`)
  ...
  const handleEditTemplate = async () => {
    ...
    setSavingTemplate(true)
    try {
      const res = await fetch(`/api/event-admin/email-templates/${eventId}/${editingTemplate}`, {
  ...
  const handleDeleteTemplate = async (id: string) => {
    try {
      const res = await fetch(`/api/event-admin/email-templates/${eventId}/${id}`, { method: 'DELETE' })
      if (!res.ok) { const d = await res.json(); notify('error', d.error || 'Failed'); return }
  ...
  const handleDuplicateTemplate = async (id: string) => {
    try {
      const res = await fetch(`/api/event-admin/email-templates/${id}/duplicate`, { method: 'POST' })
      if (!res.ok) { const d = await res.json(); notify('error', d.error || 'Failed'); return }
  ...
  const handlePreview = async (tpl: EmailTemplate) => {
    ...
    try {
      ...
      const res = await fetch(`/api/event-admin/email-templates/${tpl.id}/preview`, {
        method: 'POST', body: JSON.stringify({ variables: mockVars }),
      })
  ...
  const handleEstimateCount = async () => {
    ...
    try {
      const params = new URLSearchParams({ audience: sendAudience })
      if (sendFormat) params.set('format', sendFormat)
      const res = await fetch(`/api/event-admin/send-email?${params}`)
  ...
  const handleSendEmails = async () => {
    ...
    setSending(true)
    try {
      const res = await fetch('/api/event-admin/send-email', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleSendTest = async () => {
    if (!testTemplateId || !testRecipient) return
    setSendingTest(true)
    try {
      const res = await fetch('/api/event-admin/test-email', {
        method: 'POST', body: JSON.stringify({
          template_id: testTemplateId, recipient_email: testRecipient, variables: {}
        }),
      })
      if (!res.ok) { const d = await res.json(); notify('error', d.error || 'Failed'); setSendingTest(false); return }
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/event-admin/email-logs/${eventId}?${params}`)
      if (res.ok) { const d = await res.json(); setLogs(d.logs || []) }
    } catch {}
    finally { setLoadingLogs(false) }
  }, [eventId, logFilterTemplateType, logFilterStatus, logFilterDateFrom, logFilterDateTo])

  useEffect(() => { if (authorized && eventId) fetchLogs() }, [authorized, eventId, fetchLogs])
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: 'test-email', label: 'Test Email', icon: <Send size={14} /> },
        ...
        {activeTab === 'templates' && (
          <div>
            <div style={{ marginBottom: 16 }}>
              <button onClick={() => { setEditingTemplate(null); setTemplateForm({ template_type: 'approval', subject: '', content: '' }); setShowTemplateForm(true) }} style={s.btn}>
          ...
          <div>
            <div style={{ maxWidth: 500, ...s.card }}>
              <h3 style={{ color: '#fff', fontSize: 16, fontWeight: 600, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Eye size={16} color="#888" /> Preview Template
              </h3>
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  <label style={s.label}>Template</label>
                  <select value={testTemplateId} onChange={(e) => setTestTemplateId(e.target.value)} style={s.select}>
                    <option value="">Select a template</option>
                    {templates.map(t => <option key={t.id} value={t.id}>{t.template_type} - {t.subject || 'No subject'}</option>)}
                  </select>
                </div>
                <button onClick={() => {
                  const tpl = templates.find(t => t.id === testTemplateId)
                  if (tpl) handlePreview(tpl)
                }} disabled={previewLoading || !testTemplateId} style={{ ...s.btn, ...((previewLoading || !testTemplateId) ? { opacity: 0.6, cursor: 'not-allowed' } : {}) }}>
                  {previewLoading ? <><Loader2 size={14} className="animate-spin" style={{ marginRight: 6 }} />Loading...</> : 'Preview with Mock Data'}
                </button>
              </div>
            </div>
            ...
            {previewResult && (
              <div style={{ marginTop: 16 }}>
                <div style={{ marginBottom: 12, padding: 12, borderRadius: 6, background: '#111', border: '1px solid #222' }}>
                  <p style={{ color: '#888', fontSize: 12, marginBottom: 4 }}>Subject</p>
                  <p style={{ color: '#fff', fontSize: 14 }}>{previewResult.subject}</p>
                </div>
                <div style={{ border: '1px solid #333', borderRadius: 6, overflow: 'hidden' }}>
                  <iframe srcDoc={previewResult.content} style={{ width: '100%', height: 500, border: 'none', background: '#fff' }} title="Email Preview" />
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            <div style={{ maxWidth: 500, ...s.card }}>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                ...
                <button onClick={handleSendTest} disabled={sendingTest || !testTemplateId || !testRecipient} style={{ ...s.btn, ...((sendingTest || !testTemplateId || !testRecipient) ? { opacity: 0.6, cursor: 'not-allowed' } : {}) }}>
            ...
            <div style={{ maxWidth: 600, ...s.card }}>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                ...
                {sendTemplateType === 'reminder' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" id="includeWhatsapp" checked={sendIncludeWhatsapp} onChange={(e) => setSendIncludeWhatsapp(e.target.checked)} />
                    <label htmlFor="includeWhatsapp" style={{ color: '#ccc', fontSize: 13, cursor: 'pointer' }}>Include WhatsApp Group Link</label>
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <button onClick={handleEstimateCount} disabled={loadingCount} style={{ ...s.btnSm, background: '#88888820', color: '#ccc', ...(loadingCount ? { opacity: 0.6, cursor: 'not-allowed' } : {}) }}>
                ...
                <button onClick={() => setShowSendConfirm(true)} disabled={sendCount === 0 || sending} style={{ ...s.btn, ...((sendCount === 0 || sending) ? { opacity: 0.6, cursor: 'not-allowed' } : {}) }}>
              ...
              <div style={s.overlay} onClick={() => !sending && setShowSendConfirm(false)}>
                <div style={{ ...s.modal, maxWidth: 450 }} onClick={(e) => e.stopPropagation()}>
                  ...
                  <div style={{ padding: 16, borderRadius: 6, background: '#222', marginBottom: 16 }}>
                    ...
                    <p style={{ color: '#ccc', fontSize: 13 }}>
                      Event: <strong style={{ color: '#fff' }}>{adminName}'s Event</strong>
                    </p>
