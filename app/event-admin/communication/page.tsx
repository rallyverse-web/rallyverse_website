...
import { Mail, Plus, Edit, Trash2, Copy, Eye, Send, Loader2, RefreshCw, FileText, History, CheckCircle, XCircle, ArrowLeft } from 'lucide-react'
...
type Tab = 'templates' | 'preview' | 'test-email' | 'logs'
...
export default function EventAdminCommunicationPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setLoadingLogs(true)
    try {
      const res = await fetch(`/api/event-admin/email-logs/${eventId}`)
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: 'test-email', label: 'Send Test Email', icon: <Send size={14} /> },
